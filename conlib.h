...
static void Conlib_Vid_Mode()
{
    ...
    case 3:
        if (gfx.SetMode( Con_GetArg(1).toint(),
            ...
            gfx.IsFullScreen() ))
        {
            input.ClipMouse(SRect(0,0,Con_GetArg(1).toint(),Con_GetArg(2).toint()));
            Console_Printf("{||||||||||||}");
...
static void Conlib_Map()
{
    ...
    case 2:
        ...
        Log::Write(va("MAPswitch=%s", startmap.c_str()));
        break;
...
#  else
        Console_Printf("All rights reserved. ?/Win32 build.");
#  endif
#else
        Console_Printf("All rights reserved. GCC/Linux build.");        // assuming linux
#endif
...
static void Conlib_RString()
{
    ...
    {
    case 1:
        Console_Printf(va("Renderstring: %s", rstring.c_str()));
        Console_Printf("");
        break;
